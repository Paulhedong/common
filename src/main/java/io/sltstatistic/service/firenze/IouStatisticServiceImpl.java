...
import io.sltstatistic.mapper.firenze.custom.MyIouMapper;
import io.sltstatistic.mapper.firenze.custom.MyIouUserRecordAwardMapper;
import io.sltstatistic.mapper.firenze.custom.MyIouUserRecordPayMapper;
...
import static io.sltstatistic.constant.ConstantEnum.*;
...
public class IouStatisticServiceImpl implements IouStatisticService {
    ...
    public Map<String, Object> getTotalIouNum() {
        ...
        for (IouStatus iouStatus : IouStatus.values()){
        ...
        for(IouAppealStatus iouAppealStatus : IouAppealStatus.values()){
            boolean flag = false;
    ...
    public Map<String, Object> getSltNum() {
        ...
        sltList.forEach(m -> m.put("scene", SltAwardScene.match(Byte.valueOf(m.get("scene").toString()))));
    ...
    public Map<String, Object> getRecieveMoney() {
        ...
        incomes.forEach(
                m -> m.put("scene", PayType.getMessageByCode(Byte.valueOf(m.get("scene").toString())))
        );
